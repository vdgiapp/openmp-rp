	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(isset($_SESSION['auth_state'])) {
			if($_SESSION['auth_state'] == 0) {
				return header("location: http://hyperg.vn");
			}
		}
					...
					$result = mysqli_query($cosodulieu, $query);
		 			...
		 			if(mysqli_num_rows($result) > 0) {
		 				...
		 				if($rdata['Email'] == $email) {
		 					return warn_authmsg('Email này đã được đăng ký, vui lòng sử dụng email khác để đăng ký tài khoản!');
		 				}

		 				if($rdata['Account'] == $taikhoan) {
		 					return warn_authmsg('Tài khoản này đã được đăng ký, vui lòng sử dụng tài khoản khác để đăng ký!');
		 				}
		 			}
		 				...
		 				$query = "INSERT INTO `accounts` (`IP`, `Account`, `DisplayName`, `Email`, `Password`, `PassLv2`, `Confirm`, `CreatedDate`, `LastLogin`) VALUES ('$ip', '$taikhoan', '$taikhoan', '$email', '$matkhau', '$mkcap2', '$maxacnhan', '$regdate', '$logdate')";
